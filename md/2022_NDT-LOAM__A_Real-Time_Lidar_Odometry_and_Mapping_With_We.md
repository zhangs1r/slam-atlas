...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/dbebb026a167785bcc448cadaff6c3bfb96061309e6a426d04ba738b103ace95.jpg)

clouds and improve the accuracy. Cells of the NDT are weighted according to the range’s values and their surface characteristics, the new cost functions with weight are constructed. In the experiments, we tested NDT-LOAM on the KITTI odometry dataset and compared it with the state-of-the-art algorithm ALOAM/LOAM. NDT-LOAM had 0.899% average drift in translation, better than ALOAM and at the level of LOAM; moreover, NDT-LOAM can run at 10 Hz in real-time, while LOAM runs at 1 Hz. The results display that NDT-LOAM is a real-time and low-drift method with high accuracy. In addition, the source code is uploaded to GitHub and the download link is https://github.com/BurryChen/lv\_slam.

— Simultaneous localization and mapping (SLAM), lidar odometry, normal distributions transform (NDT), real-time.

Manuscript received November 8, 2021; accepted December 6, 2021. Date of publication December 28, 2021; date of current version February 11, 2022. This work was supported in part by the National Natural Science Foundation of China under Grant 42101445 and in part by the China Postdoctoral Science Foundation under Grant 2021M702229. The associate editor coordinating the review of this article and approving it for publication was Dr. Abhishek K. Jha. (Corresponding author: Qingquan Li.)
...
Hao Ma is with the College of Computer Science and Software Engineering, Shenzhen University, Shenzhen 518060, China (e-mail: mahao\_fido@szu.edu.cn).
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/7e593a75326f5496b26f685f5c0e5430367cfe3a657329be4ce16b162f5fea49.jpg)  
...
The Gaussian PDF in each cell can be interpreted as a generative process for the surface point p within a cell. In other words, it is originally assumed that the location of $p$ is drawn from this distribution. The mean $\mu$ and covariance - of the distribution are computed as

$$
\mu = \frac { 1 } { \mathrm { n } } \sum _ { i = 1 } ^ { n } P _ { i }\tag{1}
$$

$$
\Sigma = \frac { 1 } { n - 1 } \sum _ { i = 1 } ^ { n } \left( P _ { i } - \mu \right) \left( P _ { i } - \mu \right) ^ { T }\tag{2}
$$

where $p _ { i = 1 , \ldots , n }$ are the positions of n point samples contained in a cell. The corresponding PDF is

$$
f ( x ) { = } \frac { 1 } { ( 2 \pi ) ^ { D / 2 } \sqrt { | \Sigma | } } \exp \left( - \frac { ( P _ { i } - \mu ) ^ { T } \Sigma ^ { - 1 } ( P _ { i } - \mu ) } { 2 } \right)\tag{3}
$$
...
$$
\begin{array} { l } { { \displaystyle { F ( \xi ) = \sum _ { k = 1 } ^ { n } f ( T ( \xi , P _ { k } ) ) ) } } \ ~ } \\ { { \displaystyle ~ = \sum _ { k = 1 } ^ { n } - d _ { 1 } \cdot \exp \left( - \frac { d _ { 2 } } { 2 } \left( T ( \xi , P _ { k } ) - \mu _ { P _ { k } } \right) ^ { T } \right. } } \\ { { \displaystyle ~ \left. ~ \times ~ \Sigma ^ { - 1 } \left( T ( \xi , P _ { k } ) - \mu _ { P _ { k } } \right) \right) } } \end{array}\tag{4}
$$

$$
T ( \xi , P _ { k } ) = \exp ( \xi ^ { \wedge } ) \cdot P _ { k } = R P _ { k } + t\tag{5}
$$

where $\mu _ { P _ { k } }$ is the mean value of the cell that $P _ { k }$ belongs to. $T ( \xi , P _ { k } )$ is to transform the point $P _ { k }$ with the transformation matrix composed of the rotation matrix $\textbf { R } \in \ S \mathrm { O } ( 3 )$ and translation vector $t = [ t _ { \mathrm { x } } , t _ { \mathrm { y } } , t _ { \mathrm { z } } ]$ . In SE(3), the transformation matrix T composed of R and t can be encoded using the six-dimensional parameter vector $\xi = \bigl [ \mathrm { t _ { x } , t _ { y } , t _ { z } , } \varphi _ { \mathrm { x } } , \varphi _ { \mathrm { y } } , \varphi _ { \mathrm { z } } \bigr ] \mathrm { . }$ where $\varphi _ { \mathrm { x } } , \varphi _ { \mathrm { y } } , \varphi _ { \mathrm { z } }$ are the Euler angles or a rotation vector corresponding to the rotation matrix R. The parameters $d _ { 1 } , d _ { 2 } \mathrm { c a n }$ be calculated by the outliers constants $c _ { 1 } , c _ { 2 }$ , and they are expressed as [29]

$$
\begin{array} { r l } & { d _ { 3 } = - \log \left( c _ { 2 } \right) , } \\ & { d _ { 1 } = - \log \left( c _ { 1 } + c _ { 2 } \right) - d _ { 3 } , } \\ & { d _ { 2 } = - 2 \log \left( ( - \log \left( c _ { 1 } \exp ( - 1 / 2 ) + c _ { 2 } \right) - d _ { 3 } ) / d _ { 1 } \right) . } \end{array}\tag{6}
$$
...
Firstly, we discuss the relationship between the long measurement range and the weight. For a long measurement range comparing one nearby the sensor center, the same rotation error can cause more significant distortions in the point cloud, which means that the longer the distance, the stronger the constraint. Therefore, we introduce a range weight $\mathbf { W } _ { \Gamma }$
...
Secondly, the NDT can also be described as a method for compactly representing a surface. The transformation maps a point cloud to a smooth surface representation, described as a set of local PDFs. Each PDF with the mean $\mu$ and covariance - describes the shape and dimensionality feature of a local surface in a cell.

The covariance - in each cell is a symmetric positive definite matrix, and it can be conducted an eigenvalue decomposition for the eigenvectors and eigenvalues. The eigenvalues are positive and ordered so that $\lambda _ { 1 } \geq \lambda _ { 2 } \geq \lambda _ { 3 } > 0$ Various geometrical parameters can be derived from the eigenvalues and eigenvectors of the covariance matrix -. Several indicators have already been proposed in [30], and the following indicators have been selected to express the linear, planar, and spherical indices within a cell [31]:

$$
\begin{array} { r l } & { \forall j \in [ 1 , 3 ] , \sigma _ { j } = \sqrt { \lambda _ { j } } } \\ & { a _ { 1 D } = \frac { \sigma _ { 1 } - \sigma _ { 2 } } { \sigma _ { 1 } } } \\ & { a _ { 2 D } = \frac { \sigma _ { 2 } - \sigma _ { 3 } } { \sigma _ { 1 } } } \\ & { a _ { 3 D } = \frac { \sigma _ { 3 } } { \sigma _ { 1 } } } \end{array}\tag{8}
$$

and, the dimensionality index (1D, 2D or 3D) of a cell is defined by:

$$
d ^ { * } = \underset { d \in [ 1 , 3 ] } { \arg \operatorname* { m a x } } [ a _ { d D } ]\tag{9}
$$
...
$$
W D = \left\{ \begin{array} { l } { { 1 . 2 5 ( d ^ { * } = 2 ) } } \\ { { 1 . 0 ( d ^ { * } = 3 ) } } \\ { { 0 . 7 5 ( d ^ { * } = 1 ) } } \end{array} \right.\tag{10}
$$

Therefore, the object function of weighted NDT is

$$
\begin{array} { l } { { \displaystyle { \cal F } ( \xi ) = \sum _ { k = 1 } ^ { n } W _ { k } \cdot f ( T ( \xi , P _ { k } ) ) } } \\ { { \displaystyle ~ = - W _ { k } d _ { 1 } \sum _ { k = 1 } ^ { n } \exp \left( - \frac { d _ { 2 } } { 2 } \left( T ( \xi , P _ { k } ) - \mu _ { P _ { k } } \right) ^ { T } \right. } } \end{array}
$$

$$
\times \ \Sigma ^ { - 1 } \left( T ( \xi , P _ { k } ) - \mu _ { P _ { k } } \right) \Bigg )\tag{11}
$$

$$
W _ { k } = W r _ { k } \cdot W D _ { k }\tag{12}
$$

For brevity, let $P _ { k } ^ { \prime } \equiv T ( \xi , P _ { k } ) - \mu _ { P _ { k } }$ . In other words, $P _ { k } ^ { \prime }$ is the different between the value of the point $P _ { k }$ after transformed by the current pose parameters and the center of the cell which the point belongs to. The entries of the gradient vector $g$ is

$$
\begin{array} { l } { { \displaystyle g _ { i } = \frac { \delta F } { \delta \xi _ { i } } } } \\ { { \displaystyle \quad = \sum _ { k = 1 } ^ { n } W _ { k } d _ { 1 } d _ { 2 } \cdot P _ { k } ^ { ^ \prime } \Sigma _ { k } ^ { - 1 } \frac { \delta P ^ { \prime } } { \delta \xi _ { i } } \exp \left( - \frac { d _ { 2 } } { 2 } { P _ { k } ^ { \prime } } ^ { T } \Sigma _ { k } ^ { - 1 } P _ { k } ^ { ^ \prime } \right) } } \end{array}\tag{13}
$$

The entries of the Hessian matrix H are

$$
\begin{array} { l } { { H _ { i j } = { \frac { \delta ^ { 2 } F } { \delta \xi _ { i } \delta \xi _ { j } } } } } \\ { { \ } } \\ { { \displaystyle \quad = \sum _ { k = 1 } ^ { n } W _ { k } d _ { 1 } d _ { 2 } \cdot \exp \left( - { \frac { d _ { 2 } } { 2 } } { P } _ { k } ^ { { \mathrm { ' T } } } \Sigma _ { k } ^ { - 1 } P _ { k } ^ { \mathrm { ' } } \right) } } \\ { { \ \cdot ( - d _ { 2 } ( { P } _ { k } ^ { { \mathrm { ' T } } } \Sigma _ { k } ^ { - 1 } { \frac { \delta P _ { k } ^ { \prime } } { \delta \xi _ { i } } } ) ( { P } _ { k } ^ { { \mathrm { ' T } } } \Sigma _ { k } ^ { - 1 } { \frac { \delta P _ { k } ^ { \prime } } { \delta \xi _ { j } } } ) } } \\ { { \ \displaystyle \quad + { P } _ { k } ^ { { \mathrm { ' T } } } \Sigma _ { k } ^ { - 1 } { \frac { \delta ^ { 2 } P _ { k } ^ { \prime } } { \delta \xi _ { i } \delta \xi _ { j } } } + { \frac { \delta P _ { k } ^ { { \mathrm { ' T } } } } { \delta \xi _ { j } } } \Sigma _ { k } ^ { - 1 } { \frac { \delta P _ { k } ^ { \prime } } { \delta \xi _ { i } } } ) } } \end{array}\tag{14}
$$
...
$$
\xi  \xi - \alpha H ^ { - 1 } g\tag{15}
$$

where α is a factor determined by the line search method.

## C. Keyframe Strategy

Data accumulation and temporal inference are important steps to mitigate the drift of the odometry. Especially for a methodology with sparse point clouds as presented here, the matching with several frames make it less susceptible to errors, compared with matching among consecutive frame. Therefore, we adopt a keyframe matching strategy for the Lidar odometry. If the current scan frame $S _ { t }$ in timestamp t is matched to the last scan frame $S _ { t - 1 }$ in timestamp t-1 by NDT registration, the matching strategy of the consecutive frame is demoted as Scan2Scan. While the current scan frame $\mathbf { S } _ { \mathrm { t } }$ is matched to the recent keyframe in timestamp t-n $\left( n \geq l \right)$ , we denote the keyframe matching strategy as Scan2Key. It’s important to note that the Scan2Key strategy is just utilized in direct odometry rather than LFA, as shown in Fig. 1. In addition, we do not discuss the backend of SLAM in this paper, and the keyframes are independent of the loop closure and global optimization. For the keyframe selection, we employ a simple heuristic: if the translation relative to the last keyframe attains to a distance threshold $\varepsilon d i s$ , or the rotation relative to the last keyframe attains to a threshold $\varepsilon d e g ,$ or the time interval from the last one is more than a time threshold $\varepsilon _ { t } .$ , the frame is inserted as a keyframe. In the practical experiment, the distance threshold is usually tougher than the other two and it is the first rule of the keyframe selection.

## D. Local Feature Adjustment (LFA)

The LFA module matches features in the current scan frame $S _ { t }$ to a surrounding point cloud map $Q _ { t - 1 }$ to further refine the pose transformation. LFA consists of two steps: feature corresponding and pose adjustment. We adopt the Lidar mapping module of LOAM to implement LFA, and the reader can obtain the detailed description from [15].

In the feature corresponding, the algorithm extracts point features from the raw scan points. Point features can be either corner points or surface points. The above odometry estimation yields a 6-DoF pose estimate, which is used as an initial pose estimate for the mapping module. Using initial pose to transforms $\mathrm { S } _ { \mathrm { t } }$ into the world coordinates, denoted as $S _ { t } ^ { w }$ . To find correspondences for the feature points, we store the feature points on the map $Q _ { t - 1 }$ in the cubic areas. The points in the cubes that intersect with the transformed point cloud $S _ { t } ^ { w }$ are extracted and stored in a 3D KD-tree. We find the correspondence points in $Q _ { t - 1 }$ within a certain region around the feature points of current scan. We denote corner correspondences as $\overset { \cdot } { c } { }$ and surface correspondences as $c ^ { \mathrm { S } }$ with $c = \{ p _ { 1 } , p _ { 2 } \}$ , where $p _ { 1 }$ is a point of the current scan and $p _ { 2 }$ is a point of the map, respectively.

Once the feature correspondences are found, residual blocks of pose adjustment can be formed using the corresponding cost functions for corner and surface features, $f ^ { \mathrm { C } }$ and $f ^ { \mathrm { S } }$ respectively. The outputs of the cost functions are squared residuals, which are weighted by the loss function $\rho .$ . Residuals are the Euclidean distances between the points forming the correspondence. Residual blocks are the cost functions wrapped inside a loss function. The resulting residual blocks are added to the optimization function J . The pose adjustment problem is defined as

$$
\begin{array} { r l } { J \left( q , t \right) = \displaystyle \sum _ { c ^ { S } \in c ^ { S } } \rho \left( \left\| f ^ { S } \left( c ^ { S } , q , t \right) \right\| ^ { 2 } \right) } & { } \\ { + \displaystyle \sum _ { c ^ { C } \in c ^ { C } } \rho \left( \left\| f ^ { C } \left( c ^ { C } , q , t \right) \right\| ^ { 2 } \right) } & { } \end{array}\tag{16}
$$

where $c ^ { \mathrm { C } }$ and $c ^ { \mathrm { S } }$ are the sets of point feature correspondences for corner and surface features, respectively. $s ^ { \mathrm { C } }$ is defined as $s ^ { C } ~ = ~ \left\{ c _ { 1 } ^ { C } , c _ { 2 } ^ { C } , ~ . ~ . ~ . , c _ { n } ^ { C } \right\}$ and $\bar { s } ^ { \bar { s } }$ is defined as $s ^ { S } \ =$ $\bigl \{ c _ { 1 } ^ { S } , c _ { 2 } ^ { S } , \ldots , c _ { m } ^ { S } \bigr \}$ with n and m being the number of corner  and surface feature correspondences, respectively. q denotes the quaternion and t denotes the translation vector, which transforms the feature points of the scan from the Lidar frame to the map frame during the optimization iterations. $| | \bullet | | ^ { 2 }$ denotes the squared Euclidean distance, which is used as residual.
...
With the odometry development kit from KITTI, the odometry evaluation was carried out through computing average relative translational and rotational errors for all possible subsequences’ trajectories of length (100, . . . , 800) meters. The rotation error is measured in degrees per meter.

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/3aaf2784a104a14b1886397f4bb243f0992f9216036c989e9fa463f194d899a1.jpg)  
(a)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/e2ae9be0790572bbebcbb5d3cdbcdd2cc85f0e2071966c65292b9108fbca3abe.jpg)  
(b)  
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/3f181ec5446a120b5044ff837023781920e42b53db45cfd9d2d557f2acd3ee90.jpg)  
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/7b900097a0d73290479ca47a73357eff3e5b746d3ddd141014d266a46f96695d.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/cffb55b1a72524f0f12015fee299d5bb4c80b1cfb5aa0fe169752703446561d8.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/63743e64588a8ee302c087fcfe56858f2bdf0bef176686ce4889bb98f95041a1.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/6e82f019fa871b9e3ed272df56d77388048ba41b7440d5db0d6ac47314ccd190.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/234ca28128b511b1ec410be85034c324769419201b32327ebdf84c042ee5ecaf.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/d08640cd9f2919b0a1cb6721ef909dda5ea5fcca1e96f288130c5f1eeae4a06e.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/a0bdbfca8595f87027826c16e4490822664cbedda5253ab15bbb029cbe1cb2a7.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/8ddbcdb7c49a1ca3c195ec53aaf26c5b407621f1cf554fec9c3dd6537dd45b3c.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/5950dfbfbe5b087970e5b3cfbc5b8df0c743535730ae6c87a2b95fd9aa0ce83e.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/8ad3890c55b351bb4aa6da7f032364c420ded7a9f48fd239416c4e7580939f77.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/2fde23ef2a400c4a0d0eeabb40825162e907a56faff14fb3010581cac6e32fb1.jpg)  
Fig. 4. The initial pose/ odometry trajectory comparison between ALOAM and NDT-LOAM.

Path Length [m]  
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/db0a7674b9c8f21e025d74b922998f3d42f82ddce17909318a999e501d83a55b.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/fe9188466a6f64954c74adc43573cf8ff0bde67894878c0c0c1ab66a2428e50b.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/0e02498ace1309b3568d04e7516c003ebb29678127d66b6e76a647760bcd21f4.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/c12489eb4ab1596c268653a1316577193dc6f4590a5588f7dc562b26a9aab7fa.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/4b37a1036dc11266d8c313e035c06088701e20972c2621d03fe6f82f86c74112.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/11b90e237acb85b5a921318b53b32612e23b485b6c9611b3a51332f4644b65b7.jpg)  
Fig. 5. Point cloud map from NDT-LOAM and pose trajectories from LeGO-LOAM/NDT-LOAM on #00, #05 and #09 KITTI sequences.

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/f849b2e4209dbb0b9664d9d7f89fd76728cf3c7ac83f7127213f9f0575496f5e.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/72dfe216cb4c76640f917e6eb6ee07c62fdeff1494367a0960c08a86e4c9fb42.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/048509d2d5482ec2a906789ff9043ba24147760d4ff3b456b2b716050e5c1f51.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/bbd7b89c3961a5827adb919565bb3de41570767b4b6d04549234502db0f4a5be.jpg)  
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/488641a0c9b44d5f5e460da83795a4d3a05510c8415bccf01708a8faad2f8400.jpg)  
(a) Pose trajectories from NDT-LOAM/ LeGO-LOAM on #K1

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/37b985babb9b648a9646b6d66e3e53c4a32f5bbb9965a7fae2546e78eacd72c0.jpg)  
(b) Point cloud map from NDT-LOAM on #K1

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/e5f2b074ea23e08f21bca5a07bc9ef9236f8776b4e3597e092563ee9a6a942a8.jpg)  
(c) Pose trajectories from NDT-LOAM/ LeGO-LOAM on #K1

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/c55e5cb52e7065c090dc2a38505b2cac0ccbe9e91c36ed280655e5398572b9b8.jpg)  
(d) Point cloud map from NDT-LOAM on #K2

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/e7e4e2923b236cfa10a1a4bfef0e0e4ebaa6e3f97b32b1b175db38590e305571.jpg)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/88e6862a0200506331b685d1d3f92cef0d8f28bd02b0e6dfc9d068695a62d635.jpg)  
(e) Corresponding point cloud in left ascending staircase of (d)

![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/c9008bced5bec6a404b0b9c7f2309983ae184fe413dd895af6117d5529a3ece0.jpg)  
(f) Corresponding point cloud in right descending staircase of (d)  
Fig. 7. Pose trajectories from NDT-LOAM/ LeGO-LOAM and point cloud map from NDT-LOAM and on Kylin backpack data.

TABLE III  
ABSOLUTE POSE ERROR (m) OF NDT-LOAM AND LeGO-LOAM ON #00, #05 AND #09 KITTI SEQUENCES
...
[18] T. Shan and B. Englot, “LeGO-LOAM: Lightweight and groundoptimized lidar odometry and mapping on variable terrain,” in Proc. IEEE/RSJ Int. Conf. Intell. Robots Syst. (IROS), 2018, pp. 4758–4765.
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/0e55a3af6afe873a64568694703bc0bff4cb2ac1f669215f89c45cfcd9a144f6.jpg)  
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/ff098434767772120ef7db28f00f76e09779533467245661b01d28da6ceb52ef.jpg)
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/aa7701cabdf950cd75775dc9292e85bb499ff6e70c30621c84231fb0e275cbf3.jpg)
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/03708163d04e8c2f520249b7742badf3c19dea7384751525aeaa1cd13715c8a5.jpg)
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/e01c8b4a1266f5bb2856f573ebf47320f832b98ca799b7360e446f1367e707ce.jpg)
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/71952c7e7628565d543488adf024b8e11213804adfb183fe8106df2b4caeb933.jpg)
...
![](images/2022_NDT-LOAM__A_Real-Time_Lidar_Odometry_and_Mapping_With_We/98fd4cfd6ffc14534f28ffe8c5b1f493d324aed997a2854049df3f9bd3774dea.jpg)

Qingquan Li received the Ph.D. degree in photogrammetry and remote sensing from the Wuhan Technical University of Surveying and Mapping. He is the President of Shenzhen University. For over 20 years, Dr. Li has been engaged in the developments of surveying engineering and their innovation applications, contributing especially to the principles and practices of dynamic precision engineering survey. His research interests include the integration of GNSS, RS and GIS, precision engineering survey, and spatiotemporal big data analytics. His work has been recognized with a variety of distinctions. He is an Academician of IEAS, a member of Committee at Science and Technology Commission, Ministry of Education, China, and the Chief-Editor of the Journal of Geodesy and . His research was awarded National Scientific and Technological Progress Second Prize (China) and MoE Scientific and Technological Progress First Prize (China).