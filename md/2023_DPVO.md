...
![](images/2023_DPVO/14163aa50e91c80061da10a6d7ed199cd6b73819e0e0e6d5509a949bac7c7b06.jpg)  
...
Preliminaries: Given an input video, we represent a scene as a collection of camera poses $\mathbf { T } \in \mathbb { S E } ( 3 ) ^ { \overline { { N } } }$ and a set of square image patches P extracted from the video frames. Using d to represent inverse depth and $\displaystyle ( \mathbf { x } , \mathbf { y } )$ to represent pixel coordinates, we represent each patch as the $4 \times p ^ { 2 }$ homogeneous array

$$
\mathbf { P } _ { k } = \left( \begin{array} { l } { \mathbf { x } } \\ { \mathbf { y } } \\ { 1 } \\ { \mathbf { d } } \end{array} \right) \qquad \mathbf { x } , \mathbf { y } , \mathbf { d } \in \mathbb { R } ^ { 1 \times p ^ { 2 } }\tag{1}
$$

where $p$ is the width of the patch. We assume a constant depth for the full patch, meaning that it forms a frontoparallel plane in the frame from which it was extracted. Letting i denote the index of the sourceframe of the patch, i.e. the frame from which which patch $\mathbf { P } _ { k }$ was extracted, we can reproject the patch onto another frame $j$

$$
\mathbf { P } _ { k j } ^ { \prime } \sim \mathbf { K } \mathbf { T } _ { j } \mathbf { T } _ { i } ^ { - 1 } \mathbf { K } ^ { - 1 } \mathbf { P } _ { k } .\tag{2}
$$
...
The pixel coordinates $\mathbf { x } ^ { \prime } = ( x ^ { \prime } , y ^ { \prime } )$ can be recovered by dividing by the third element. For the rest of the paper, we use the shorthand $\mathbf { P } _ { k j } ^ { \prime } = \omega _ { i j } ( \mathbf { T } , \mathbf { P } _ { k } )$ to denote the reprojection of patch k onto frame j in terms of pixel coordinates.
...
![](images/2023_DPVO/3533b60a228881dbc4873f9a1e7523f93bd13fd9bc193c6d50b855c7ec839379.jpg)  
Figure 2: Feature and Patch Extraction (Sec. 3). Residual networks extract 1) a context feature map at $\frac { 1 } { 4 }$ resolution and 2) a 2-level pyramid of matching features at $\textstyle { \frac { 1 } { 4 } }$ and $\frac { 1 } { 8 }$ resolution. Many p×p patches are cropped from this feature map at random pixel coordinates using bilinear sampling.
...
![](images/2023_DPVO/ec1fe7e7846755d976083df3f59621d960a9ee7c7926b6cb7fcb0fd2c2623d56.jpg)  
...
Correlation: For each edge $( k , j )$ in the patch graph, we compute correlation features (visual similarities) to assess the visual alignment given by current estimates of depth and poses and to propose revisions. We first use Eqn. 2 to reproject patch k from frame i into frame j: $\mathbf { P } _ { k j } ^ { \prime } = \omega _ { i j } ( \mathbf { T } , \mathbf { P } _ { k } )$ Given patch features $\textbf { g } \in \mathbb { R } ^ { p \times p \times D }$ and frame features $\mathbf { f } \in \mathbb { R } ^ { \pmb { \tilde { H } } \times \pmb { W } \times \pmb { D } }$ , for each pixel $( u , v )$ in patch k, we compute its correlation $\mathbf { C } \in \mathbb { R } ^ { p \times p \times 7 \times 7 }$ with a grid of pixels centered at its reprojection in frame j, using the inner product:

$$
\mathbf { C } _ { u v \alpha \beta } = \langle \mathbf { g } _ { u v } , \mathbf { f } ( \mathbf { P } _ { k j } ^ { \prime } ( u , v ) + \Delta _ { \alpha \beta } ) \rangle\tag{4}
$$

where we take $\Delta$ to be a $7 \times 7$ integer grid centered at $_ 0$ indexed by α and $\beta ,$ and $\mathbf { f } \left( \cdot \right)$ denotes bilinear sampling.
...
$$
\psi \left( \left[ \sum _ { \stackrel { x \in N ( e ) } { x \in N ( e ) } } \sigma ( x ) \cdot \phi ( x ) \right] / \sum _ { x \in N ( e ) } \sigma ( x ) \right)\tag{5}
$$
...
Pose Supervision: We scale the predicted trajectory to match the groundtruth using the Umeyama alignment algorithm [40]. Then for every pair of poses $( i , j )$ , we supervise on the error

$$
\sum _ { ( i , j ) \ i \neq j } \| { L o g _ { S E ( 3 ) } [ ( \mathbf { G } _ { i } ^ { - 1 } \mathbf { G } _ { j } ) ^ { - 1 } ( \mathbf { T } _ { i } ^ { - 1 } \mathbf { T } _ { j } ) } ] \|\tag{7}
$$

where G is the ground truth and T are the predicted poses.

Flow Supervision: We additionally supervise on the distance between the induced optical flow and the ground truth optical flow between each patch and the frames within two timestamps of its source frame. Each patch induces ${ \bf a } p \times { \boldsymbol { p } }$ flow field. We take the minimum of all $p \times p$ errors.

The final loss is the weighted combination

$$
\mathcal { L } = 1 0 \mathcal { L } _ { p o s e } + 0 . 1 \mathcal { L } _ { f l o w } .\tag{8}
$$
...
![](images/2023_DPVO/964a5c5233ff3d51e2d2c6cdcdcb44ec1a9c13456e9621b0981e10de2b4b154b.jpg)  
...
![](images/2023_DPVO/6ed7d6f8a83dca50b02ff573aab88bfa4c3b976cc4c0eded221a427cbd9f02b8.jpg)  
...
![](images/2023_DPVO/78fe280dc717883a9f4b524138612459f073602bae8e714a6253bdadc1b63edf.jpg)  
...
![](images/2023_DPVO/88505e5aac3ce9b87eea5869ba555791654498eac10d1cd69dd050f1ac192c0b.jpg)  
...
![](images/2023_DPVO/781274195bb3cee8b91fbe8ef5e6e00e8e5b1d5f42e8e224d47142b5812e79d5.jpg)  
(a) Point vs Patch features

![](images/2023_DPVO/c90e3ece909f69e92b2c0126da54ade5fa26379fef85edf54afc53a6a721414a.jpg)  
(b) Update operator components

![](images/2023_DPVO/3b011e67cccd1e450d06f644594cb6f2dc9bb6fc7a4c3268b12ede0eed7523c7.jpg)  
...
![](images/2023_DPVO/9b73adbddd85875a9f2d095ac9a90ec934d3c162d6aaaec53b15312dacd8de65.jpg)  
...
![](images/2023_DPVO/848564e0cbad474ed1ae4ec369d78f6345f88b778948e84b6477d66efa4f58df.jpg)  
...
![](images/2023_DPVO/4569e753cc61968309cdbe374f0491ceb6661050b19b7744d5d6033e43016e95.jpg)  
...
We compare results using the ATE (average trajectory error) metric, which is standard for VO and SLAM [16, 37, 27, 43, 12]. The ATE metric between the predicted trajectory T<sup>˜</sup> and the ground-truth trajectory T<sup>ˆ</sup> is computed using

$$
E ( \mathbf { \tilde { T } } , \mathbf { \hat { T } } ) : = \sum _ { t = 1 } ^ { N } | | \mathbf { \tilde { T } } _ { x y z } - \mathbf { \hat { T } } _ { x y z } | | _ { 2 }\tag{9}
$$
...
DPVO is trained entirely on TartanAir [44]<sup>1</sup>, a synthetic dataset. This is the same synthetic training data used by previous VO systems [43, 37]. TartanAir provides a large number of training scenes of both indoor and outdoor environments with varied lighting and weather conditions. The dataset provides depth and camera pose annotations, enabling one to generate optical flow by re-projecting the depth using the camera poses. All dataset parameters are identical to those used in DROID-SLAM [37]. This includes random cropping, color-jitter, random greyscale, random color inversion, as well as re-scaling the training scenes in order to improve training stability.

<table><tr><td></td><td>Living Room 0</td><td>Living Room 1</td><td>Living Room 2</td><td>Living Room 3</td><td>Office Room 0</td><td>Office Room 1</td><td>Office Room 2</td><td>Office Room 3</td><td>Avg</td></tr><tr><td>ORB-SLAM2* [27]</td><td>0.461</td><td>0.172</td><td>0.806</td><td>N/A</td><td>0.589</td><td>0.813</td><td>1.000</td><td>N/A</td><td>N/A</td></tr><tr><td>DROID-SLAM* [37]</td><td>0.008</td><td>0.027</td><td>0.039</td><td>0.012</td><td>0.065</td><td>0.025</td><td>0.858</td><td>0.481</td><td>0.189</td></tr><tr><td>DROID-VO [37]</td><td>0.010</td><td>0.123</td><td>0.072</td><td>0.032</td><td>0.095</td><td>0.041</td><td>0.842</td><td>0.504</td><td>0.215</td></tr><tr><td>SVO [15]</td><td>0.02</td><td>0.07</td><td>0.09</td><td>0.07</td><td>0.34</td><td>0.28</td><td>0.14</td><td>0.08</td><td>0.136</td></tr><tr><td>DSO [12]</td><td>0.01</td><td>0.02</td><td>0.06</td><td>0.03</td><td>0.21</td><td>0.83</td><td>0.36</td><td>0.64</td><td>0.270</td></tr><tr><td>DSO (Realtime) [12]</td><td>0.02</td><td>0.03</td><td>0.33</td><td>0.06</td><td>0.29</td><td>0.64</td><td>0.23</td><td>0.46</td><td>0.258</td></tr><tr><td>Ours (Default)</td><td>0.006</td><td>0.006</td><td>0.023</td><td>0.010</td><td>0.067</td><td>0.012</td><td>0.017</td><td>0.635</td><td>0.097</td></tr><tr><td>Ours (Fast)</td><td>0.008</td><td>0.007</td><td>0.021</td><td>0.010</td><td>0.071</td><td>0.015</td><td>0.018</td><td>0.593</td><td>0.093</td></tr></table>
...
![](images/2023_DPVO/bb88b8de91b520fceb4ccf326ef241205779a105df0a147714fd3cdb8c69ec2b.jpg)  
Figure C: Reconstruction on the ETH-3D [32] dataset.

## E. Visualization

Reconstructions are visualized interactively using a separate visualization thread. Our visualizer is implemented using the Pangolin library<sup>2</sup>. It directly reads from Py-Torch tensors avoiding all unnecessary memory copies from CPU to GPU. This means that the visualizer has very little overhead–only slowing the full system down by approximately 10%. In Fig. C, we show a reconstruction on the ETH-3D [32] dataset, and in Fig. B we show a reconstruction from our video demo.

## F. Bundle Adjustment Layer

Our bundle adjustment layer is functionally identical to the BA layer in DROID-SLAM [37], except that DROID-SLAM predicts the damping factor using its update operator, while we found this not to be necessary and hardcode it to $1 0 ^ { - 4 }$ instead. We refer the reader to Appendix C in the DROID-SLAM paper for the analytical gradients of the Gauss-Newton update iterations. Since DROID-SLAM uses dense flow, we implement our own sparse version as an optimized CUDA layer.

![](images/2023_DPVO/3f36604234f7ba2acd72944711ca74aac73f2651a00295bbcdfc008d427080a8.jpg)  
...
![](images/2023_DPVO/5cee0f49b74fc732b88639c2e036cf89c74cfff84f575f25e237510ce376a7f3.jpg)
...
![](images/2023_DPVO/74f3e7a630d7efc8b48b58d765e857f109030be42e369277622bf1aeee561fff.jpg)  
Figure F: The correlation step of the update operator. For each edge $( k , j )$ in the patch graph, patch k is reprojected into a frame j using equation 2. Frame j’s matching features are then cropped using the reprojected patch. The correlated matching features (blue, green) and context features (grey) form the edge features. Each dot in the sampled matching features (green) represents a $7 \times 7$ neighborhood of points, however this is omitted for clarity.
...
![](images/2023_DPVO/516c91d5b4c38508d5afde73be6e51f582302c7f1450d9a53b2854b3d9800e18.jpg)

Figure G: A high-level view of the update operator following Fig. F. Correlation and context features are extracted for each edge (k, j). Several learnable layers (yellow) are used to predict an update $\delta _ { ( k , j ) } \in \mathbb { R } ^ { 2 }$ to the flow, a confidence weight $\Sigma _ { ( k , j ) } \in \mathbb { R } ^ { 2 }$ , and an update to the hidden state $h _ { k , j } ^ { t } .$ The predicted factors are used in the bundle adjustment layer to produce an update to the camera poses and patch depths. The factor head is omitted here for clarity.  
![](images/2023_DPVO/5776beaad61d8b7b84bdbc85568d3db3806d3df2da2776e6a204e5628244aff5.jpg)  
Figure H: Factor confidence weights. For each edge in the patch graph, the factor-head of our update operator predicts a confidence weight w $\in \mathbb { R } ^ { 2 }$ bounded to (0, 1) and a 2D flow update. Cold colors (blue) represent high confidence edges while hot colors (red) represent low confidence edges.
...
Feature Extractor: In Fig. J, we show a visualization of the architecture of the feature extractor. The same network architecture is used for extracting context features and for extracting matching features, except the former uses no normalization and output dimension $D = 3 8 4$ while the latter uses instance normalization and $D = 1 2 8$ . We use instance normalization for the matching features since they should be calculated independently for each input image in a batch, which other flow-based networks [36, 38, 37] do as well. Our feature extractor is similar to the architecture used in DROID-SLAM [37], but half the size. Consequently, the output resolution is 1/4 of the image resolution, as opposed to 1/8th. Since DPVO only tracks a sparse collection of patches instead of predicting dense flow, we can afford to use higher spatial-resolution features without significant memory overhead.

![](images/2023_DPVO/fbb4ed1211cbc9a8253ef3406cf12feb60d9305d2e7ae52b8edaad85a4f89b04.jpg)  
...
![](images/2023_DPVO/db8e4f1aabecf8c4f70d3e114fba370a9936a292e7b0c83bb1dad6ee0498b7f1.jpg)  
