...
## 十五、第十一季（阶段 K 应用与相邻领域）完成（2026-09-21）

5 篇 → **7 节课（`0199`–`0205`）**。**★ 这是整个项目的最后一季，A–K 十一个阶段至此全部开课。**
**30 张具象自绘图**（每节 4.3 张，各季最高）、**1013 个 SVG 文字标注**、28 张原文图。

### 72. ★★ 第五种季型：「横切·看位置季」——导览课再重新设计一次

| 季型 | 代表 | 导览课要讲什么 |
|---|---|---|
| **续接季** | A–G | 预告「一个技术演进」的下一站 |
| **横切·支线季** | H | 讲清「横切在哪、拆掉了什么默认假设」 |
| **横切·工具季** | I | 先立一套分析框架（评测坐标系四维度） |
| **横切·补名词季** | J | 先承认「这些词你早就在论文里见过、但没人解释过」 |
| **横切·看位置季** | **K** | **先画出「一个完整机器人系统」的框图**，再说明 SLAM 只占其中一环 |

**框架是一条七环系统链**：`传感器 → 感知/前端 → SLAM/里程计 → 地图 → 规划 → 控制 → 执行器`。
**每节课都必须回答「它在链的哪一环、上游是谁、下游给谁」。**

**★ 本季最有教学价值的一点：五篇覆盖的环极不均匀，而这本身就是要讲的内容。**
`0200` 横跨六环（唯一自洽闭环、唯一落到执行器）；`0201` **一环都不实现**（导航栈的解剖图）；
`0202` 只管 SLAM 与地图两环的「算力部署与融合方式」（算法零改动）；
`0203` 就是 SLAM 这一环本身；`0204` 在感知这一环。
→ **经验：横切季的框架不一定要「覆盖齐」，把「覆盖得不齐」讲清楚往往更有信息量。**

### 73. ★★ 第 20 处：文件名截断造成了一次实质性误读

清单标「边缘计算加速多机器人**仿真**」——**错**。真实标题是
`Edge Robotics: Edge-Computing-Accelerated Multirobot **Simultaneous Localization and Mapping**`。

**误读链条**：文件名 `..._Multirobot_Sim.md` 里的 `_Sim`
是 **`Simultaneous` 被 66 字符长度限制切掉后剩下的残骸**，**正好切在第三个字母上**；
于是 `Simultaneous` → `Sim` → 被顺带读成 `simulation`。
**而且它不只是仿真**：有真机原型（3 台 TurtleBot3 + 1 个边缘节点，端侧 Jetson TX2）。

**★ 本季三篇应用论文的文件名都在第 66 个字符处被截断**——这是**系统性失真，不是个别事故**。
→ **新判定动作：清单一篇论文的名字如果读起来「像是某个常见词的缩写」，先去对一级标题。**
   本季另两篇也因此少了关键信息（`Under Dense Forest Canopy`、`using machine learning: a survey`）。

### 74. ★★ 第 21 处：清单「替论文说了一句它没说过的话」

清单在 Wildcat 备注里写「**用 SE(2) 相对位姿做稠密重建**」。逐词实测：
**`SE(2)` 在正文出现 0 次**（出现的是 `SE(3)` / `SO(3)` / `so(3)`），
**也没有「稠密重建」这个说法**（产物是 **surfel + submap**）。

→ **处理纪律（本季新固化的一条，值得单独记）**：
**「这是清单的说法，不是论文写的；我们不为它补话。」**
不去替论文解释「它其实也想用 SE(2)」，也不去替它圆「稠密重建大概是这个意思」。
**论文说的是什么，我们就写什么。**
→ 并顺手把 SE(2)/SE(3) 讲清，附**三条「为什么不可能是 SE(2)」的正文依据**
（场景三维含楼梯隧道 / 位姿图有重力方向约束 / 面元代价是三维 point-to-plane），
还附一个**误读来源的合理猜测**（它用协方差最小特征值判断面元「平不平」——
那是**面元**的局部几何，不是**位姿**被表示成平面运动；**两个「平面」很容易串**）。

### 75. ★★★ 撤稿警示课：本季最需要纪律的一节（`0204`）

**事实（逐词实测）**：正文**从头到尾没有撤稿声明**——`retract` 全文命中 **27 处，
27 处全部落在图片路径里**（图片目录名继承自带 `RETRACTED_ARTICLE` 前缀的文件名，是假命中）；
**排除图片路径后正文文字命中 0 处**。Declarations 段反而**完全合规**。
外部核实（Springer 官网）：标题前缀 `RETRACTED ARTICLE`、Soft Computing 卷 27 期 23 页 18195–18213、
DOI `10.1007/s00500-023-09278-3`、变更历史「09 March 2026 — This article has been retracted」、
撤稿通知另见 DOI `10.1007/s00500-026-11323-w`。**它仍有 98 条 Scopus 引用。**

**课上的四段骨架**：起（零件全对，全是第十季讲过的）→ 承（Declarations 四条逐字抄，全合规）→
转（回论文内部查自洽性：97.13% vs 85.67%、70 ms vs 15.8 FPS、4400 vs 4500、
对比表引用归属系统性对不上、式(11) 底行全零、式(10) 的 R 没进目标函数）→
合（判据换成「可核查性」，并给出发现之后的处置）。

**★★★ 不许说的话（写进规范 §八④，逐条执行）**：
不许写「因为数据造假 / 无法复现 / 作者有问题 / 审稿人发现了什么所以被撤」；
**不许出现「撤稿原因是……」这一句式**；
不许把「读起来不对劲的地方」说成「撤稿的证据」，只能说「读时应当起疑、应去核查的地方」。
需要一句话时用：「**正文未给出撤稿理由；本课不写理由。**」

**机检办法**：把禁止句式做成清单去 grep 成品课页。
★ 但要注意**反向命中**——本季机检报「撤稿的证据」命中 3 次，人工核对发现**3 次全是否定句**
（「不是『撤稿的证据』」「不叫『撤稿的证据』」），是**正确用法**。
**→ 经验：关键词机检必须回看上下文，否定句式会被误报。**

**★ 全季最重要的一句话**：**零件都对、声明都合规、格式都像样，仍不保证整篇可信。**
判据要建立在「可核查性」上——这也解释了那 98 条引用：
**撤稿不会自动撤回别人已经引用的行为。**

### 76. ★★ 本季新踩的坑：写作 agent 会「编」跨季链接的文件名

**首轮机检抓到 4 节课共 21 处跨季链接全是死链。** 链接文字语义都对得上
（「第八季 0160–0167 那块」「第四季 0073 起」），但文件名是**按课号 + 自己猜的标题拼出来的**，
不是去 `ls` 真实文件名。

→ **修法**：按**课号**换成真实文件名（上下文本身就是「X 起那一块」，按课号替换语义正确）。
写了 `_tmp/fix_deadlinks_k.py`，21 处一次修完，复检断链 0。

→ **★ 流程教训（从前几季的「目录锚点」升级版）**：
**写作 agent 提示里那句「照 `k_lesson_list.txt` 抄文件名」只管本季的导航，
管不住「正文里引用往季课号」这件事。**
→ **下一季（如有）应在提示里明确加一条**：
**「凡在正文里链接往季课号，必须先用 `ls course/lessons/<课号>-*.html` 取真实文件名，不许按课号猜标题。」**
→ 并且**这条已经由 `validate_delivery.py` 兜住**——它正是靠「断链检查」抓到的。
**「把校验做成一条命令、每季都跑」的价值，再一次被证明。**

### 76b. 本季最终实测数据

```
audit_lessons.py 0199 0205            → 7 节，问题项 0
                                      （30 张自绘图 / 1013 个 SVG 文字标注 / 28 张原文图）
svgcheck.py（全季 7 节 + 新成就卡）     → 0 OVERLAP / 0 OUT-OF-BOX
...
**另一处自检脚本自身写错的教训**：`reg_card_k.py` 里的「引号配平自检」
一开始写成 `assert seg.count('"%s",') == 1` —— 但 `%s` 在断言前**已经被替换掉了**，
字符串里根本不存在字面量 `"%s",`，于是**断言永远失败**。
→ **经验：对「替换之后的结果」做自检时，要断言替换后的形态**（用正则匹配
`"data:image...utf8," + "…<svg …>",` 这个结构），不是断言替换前的模板占位符。
...
- **主干七季（A–G）**：纵向技术演进。从全景纲领 → 打地基 → 视觉主干 → 激光主干 →
  多传感器融合 → 神经隐式与 3D 高斯 → 学习式方法。**从全景出发，最后回到全景。**
- **四个横切面（H–K）**：四种不同的切法。**H 拆假设 · I 教衡量 · J 补名词 · K 看位置。**

**这十一季留下的、最可复用的东西是「五步工作流 + 五种季型对照表」**：
