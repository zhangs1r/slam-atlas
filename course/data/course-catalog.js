...
window.PAPERLESSON_DATA = {
  ...
  groups: [
    {
      ...
      meta: ["按阶段分块", "255 篇语料库", "全十一季已开课（A–K 十一个阶段）"]
    },
    {
      id: "qa",
      kind: "答疑馆",
  ...
  collections: [
    {
      ...
      quickOpen: "lessons/0001-导览-255篇论文的阅读地图.html",
    ...
    },
    {
      ...
      quickOpen: "lessons/0017-导览-第二季开场-打地基.html",
    ...
    },
    {
      ...
      quickOpen: "lessons/0040-导览-第三季开场-视觉主干.html",
    ...
    },
    {
      ...
      quickOpen: "lessons/0073-导览-第四季开场-激光主干.html",
    ...
    },
    {
      ...
      quickOpen: "lessons/0095-导览-第五季开场-三传感器怎么合成一台机器.html",
    ...
    },
    {
      ...
      quickOpen: "lessons/0104-导览-第六季开场-隐式与显式之争.html",
      meta: ["24 节课", "覆盖 39 篇论文", "49 张具象自绘图"]
    },
    {
      id: "stage-g",
      groupId: "papers",
      ...
      quickOpen: "lessons/0128-导览-第七季开场-一句话的面试.html",
    ...
    },
    {
      ...
      quickOpen: "lessons/0152-导览-第八季开场-三个不成立的假设.html",
    ...
    },
    {
      ...
      kind: "阶段精读",
      title: "阶段 I · 数据集与评测",
      subtitle: "数据集与评测 · 第九季 5 篇 / 7 节",
      description: "第 0179 课起。前八季讲方法，这一季只讲尺子——怎么衡量一个 SLAM 做得好不好。全季主线是一句话：真值从哪来，决定了这把尺子量得出什么、量不出什么。五篇按真值来源正好排成阶梯：动捕与激光跟踪仪（毫米级但空间小）、毫米级靶标加扫描仪、测绘扫描仪加 ICP（厘米级）、RTK-GPS（厘米级但要看得见天）、以及 SLAM 自举（亚米级，只能评检索）。其中只有 TUM 2012 是评测方法论文，ATE 与 RPE 两个指标正是它定义的；其余四篇是数据集。核心洞见：够用比准更重要——任务不同，对真值精度的要求就不同。",
      quickOpen: "lessons/0179-导览-第九季开场-怎么衡量一个SLAM好不好.html",
      meta: ["7 节课", "覆盖 5 篇论文", "一把尺子"]
    },
    {
      id: "stage-j",
      groupId: "papers",
      kind: "阶段精读",
      title: "阶段 J · 基础零件",
      subtitle: "基础零件 · 第十季 11 篇 / 13 节",
      description: "第 0186 课起。前八季讲的都是方法，这一季换成第四种季型：补名词。11 篇里没有一篇是 SLAM 论文，全是计算机视觉与深度学习的通用零件——骨干网络、检测头、分割头，加上两端的硬件与三维检测综述。可它们在前八季的论文里到处出现，读者却从没被解释过。全季按四个家族组织：骨架（GoogLeNet、ResNet、ResNeXt、Swin、PVT）负责网络的躯干，检测头（YOLO、Focal Loss）负责把物体框出来，分割头（FCN、Mask R-CNN）负责把像素涂上类别，两端的外围（LiDAR 硬件回答点云从哪来、三维检测综述回答点云里的框怎么画）。另有一条暗线贯穿全季：语义前端 195 毫秒一帧约等于 5 帧每秒，而 SLAM 要实时——本季所有零件的落地摩擦都在这条线上。三处清单与正文不符都在课里显式纠正：清单说的 VGG 实为 GoogLeNet、清单说的 PVTv2 实为 PVT v1、以及综述标题是 Survey 且全文零次出现 SLAM。",
      quickOpen: "lessons/0186-导览-第十季开场-论文里那些没人解释的名词.html",
      meta: ["13 节课", "覆盖 11 篇论文", "四族零件"]
    },
    {
      id: "stage-k",
      groupId: "papers",
      kind: "阶段精读",
      title: "阶段 K · 应用与相邻领域",
      subtitle: "应用与相邻领域 · 第十一季 5 篇 / 7 节",
      description: "第 0199 课起，也是全项目最后一个横切面。它不读新方法，只回答一个问题：SLAM 在一个完整的机器人系统里站在哪一环，上游从哪拿数据、下游把结果交给谁。全季的分析框架是一条七环系统链：传感器、感知前端、SLAM 与里程计、地图、规划、控制、执行器。五篇覆盖的环极不均匀，这本身就是要讲的内容——Falcon 4 横跨六环、是唯一自洽闭环并落到执行器的整机系统；运动规划综述一环都不实现，它是导航栈的解剖图；RecSLAM 只管 SLAM 与地图两环的算力部署与融合方式，算法零改动；Wildcat 就是 SLAM 与里程计这一环本身，并与第九季 Wild-Places 的真值来源闭环；撤稿的那篇在感知这一环，专门上成一节学术诚信操作课。三处清单与正文不符都在课里显式纠正，其中一处是把 Simultaneous 的截断误读成仿真。",
      quickOpen: "lessons/0199-导览-第十一季开场-SLAM站在系统的哪一环.html",
      meta: ["7 节课", "覆盖 5 篇论文", "看位置"]
    },
    {
      id: "qa-0002",
      groupId: "qa",
      ...
      quickOpen: "qa/qa-0002-MAP到非线性最小二乘.html",
    ...
    },
    {
      ...
      quickOpen: "../outputs/论文阅读路线图.html",
  ...
  lessons: [
    ...
    {
      id: "0179",
      path: "lessons/0179-导览-第九季开场-怎么衡量一个SLAM好不好.html",
      paper: "stage-i",
      title: "导览：怎么衡量一个 SLAM 好不好",
      subtitle: "第九季开场·只讲尺子不讲方法",
      emoji: "📏",
      duration: "55 分钟",
      tags: ["导览", "数据集与评测", "评测坐标系"],
      description: "第九季开场。前八季讲方法，这一季只讲怎么衡量方法的好坏。含一张四维评测坐标系图、以及一张真值精度阶梯图：把毫米级、厘米级、亚米级三档刻度配上对应的真实设备（动捕房的红外相机阵列、全站仪三脚架、机器人顶上的 RTK 天线、测绘扫描仪），并注明各自能覆盖多大空间。开门见山回链第一季 0013 课那句关于评测口径的警告。"
    },
    {
      id: "0180",
      path: "lessons/0180-TUM-RGBD-2012-ATE与RPE的出处.html",
      paper: "stage-i",
      title: "TUM RGB-D（2012）：ATE 与 RPE 是这么来的",
      subtitle: "全季唯一的评测方法论文",
      emoji: "📐",
      duration: "75 分钟",
      tags: ["单篇", "评测方法", "ATE与RPE", "重点"],
      description: "全季技术含量最高的一节：ATE 与 RPE 这两个沿用至今的指标就是它定义的。含真实走廊上的轨迹对照图（竖线长度就是那一刻的误差）、逐段比较的 RPE 图、以及一张「整条偏移但形状对」与「整体位置对但中间飘」的并排对照图，把两个指标的分工讲透。另讲轨迹对齐方式 SE(3) 与 Sim(3) 的区别（单目天生缺尺度，必须用 Sim(3) 才公平）、时间戳关联、以及 RMSE 与中位数为何有时都报。"
    },
    {
      id: "0181",
      path: "lessons/0181-NewerCollege2020-手持激光与退化场景.html",
      paper: "stage-i",
      title: "Newer College（2020）：手持激光与退化场景",
      subtitle: "厘米级真值的大规模参考",
      emoji: "🎒",
      duration: "55 分钟",
      tags: ["单篇", "数据集", "手持激光", "退化"],
      description: "手持传感器数据集的代表。真值来自测绘级激光扫描仪建图加 ICP 注册，精度约 3 厘米——课上讲清为什么这个场景做不到毫米级。重点是退化场景：含一张真实长走廊的透视图（两侧白墙、激光与视觉同时失去信息），说明几何退化到底意味着什么。回链第四季激光主干里关于退化的讨论。"
    },
    {
      id: "0182",
      path: "lessons/0182-M2DGR-2022-地面机器人的致命场景.html",
      paper: "stage-i",
      title: "M2DGR（2022）：地面机器人的致命场景",
      subtitle: "一份数据集里三种真值",
      emoji: "🛗",
      duration: "55 分钟",
      tags: ["单篇", "数据集", "地面机器人", "多源真值"],
      description: "地面机器人多传感器数据集。题眼是它的真值不是单一来源，而是按场景切换：室内动捕约 1 毫米、激光跟踪仪约 1 毫米加 1.5 ppm、户外 RTK 约 2 厘米——所以不能笼统说整份数据集的真值是毫米级。场景专挑致命情形，含电梯内、全黑环境、室内外切换的连环画，另有一张 RTK 天线与「头顶看不见天」的对照图。"
    },
    {
      id: "0183",
      path: "lessons/0183-Hilti-Oxford-2023-毫米级真值.html",
      paper: "stage-i",
      title: "Hilti-Oxford（2023）：把评测推向绝对精度",
      subtitle: "毫米级真值是分区域的",
      emoji: "🎯",
      duration: "55 分钟",
      tags: ["单篇", "数据集", "毫米级真值", "挑战赛"],
      description: "把评测从相对排名推向绝对精度。毫米级真值靠布置 AprilTag 靶标实现，靶标处毫米级、靶标之间靠插值，所以精度是分区域的——含一张真实墙面加三个靶标的图，标出靶标附近准、两靶标之间精度下降。另含一张评分函数天平图，把精度与是否跟丢放在一起打分，说明跟丢扣分远比不准重。场景含长走廊、窄楼梯、暗角等专逼现有方法到极限的序列。"
    },
    {
      id: "0184",
      path: "lessons/0184-WildPlaces-2023-自然场景的地点识别.html",
      paper: "stage-i",
      title: "Wild-Places（2023）：亚米级真值也能评地点识别",
      subtitle: "全季最重要的那处纠正",
      emoji: "🌲",
      duration: "60 分钟",
      tags: ["单篇", "数据集", "地点识别", "纠正"],
      description: "全季最需要小心的一节，也是一处必须显式纠正的地方：它的真值只有亚米级——由 Wildcat SLAM 自举生成，原文写明平均误差 0.3 米；机间配准「95% 对应点误差小于 0.95 米」；地点识别的成功判据是检索结果落在 3 米内。因此它不能与 TUM、Hilti 放在同一精度梯队。课上把这层道理讲透：任务不同，对真值精度的要求就不同，它评的是检索、判据本就是米级，所以亚米级完全够用——真值越准越好是外行话，够用才是工程标准。另含用 SLAM 评 SLAM 的循环论证风险、共享点泄漏与 1 秒时间窗，并回链第七季的 Recall@1 与判据半径这个前提。"
    },
    {
      id: "0185",
      path: "lessons/0185-收官-真值精度阶梯与选尺子指南.html",
      paper: "stage-i",
      title: "收官：真值精度阶梯与选尺子指南",
      subtitle: "排行榜数字为什么不可横比",
      emoji: "🧾",
      duration: "60 分钟",
      tags: ["收官", "数据集与评测", "总收束"],
      description: "全季收官，结账四件事：把五篇按真值精度排成毫米、厘米、亚米三级阶梯并说明各自适用边界；给一张选尺子决策表（想测什么任务就选哪个数据集）；正面回答排行榜上的数字为什么不可直接横比，给出至少四条理由（真值精度不同、轨迹对齐方式不同、序列场景不同、RMSE 与中位数的选择不同、以及跟丢了算不算的规则不同）；最后诚实分区，标出哪些结论有原文证据、哪些只是判断。"
    },
    {
      id: "0186",
      path: "lessons/0186-导览-第十季开场-论文里那些没人解释的名词.html",
      paper: "stage-j",
      title: "第十季开场：论文里那些没人解释的名词",
      subtitle: "承认欠账·零件地图·精度与实时性的暗线",
      emoji: "🗺️",
      duration: "55 分钟",
      tags: ["导览", "基础零件", "名词地图"],
      description: "第十季开场。11 篇里没有一篇是 SLAM 论文，全是 CV 与深度学习的通用零件，但前八季的论文里到处在引用它们。导览承担五件事：先说破处境——读者读前八季时一定跳过了一堆词（backbone、mAP、IoU、RoIAlign、focal loss），这不是读者的问题，是论文从来不解释；再回链具体的欠账现场（第八季动态块的 Mask R-CNN 与 YOLO、语义块的语义分割、第三季与第七季的骨干网络）；然后立一张零件地图，把 11 篇按骨架、检测头、分割头、两端外围四个家族分好，并画出它们各自插在 SLAM 流水线的哪一段；再画出全季的暗线：语义前端 195 毫秒一帧约等于 5 帧每秒，而 SLAM 要实时，本季所有零件的落地摩擦都在这条线上；最后给出全季课表，并预告收官课要回答的问题——看到一个陌生的词，怎么在三十秒内判断需不需要读懂它。"
    },
    {
      id: "0187",
      path: "lessons/0187-VGG与GoogLeNet-2015-两条经典骨干.html",
      paper: "stage-j",
      title: "VGG 与 GoogLeNet（2015）：两条经典骨干",
      subtitle: "清单说 VGG，正文其实是 GoogLeNet",
      emoji: "🧩",
      duration: "60 分钟",
      tags: ["单篇", "骨干", "Inception", "纠正"],
      description: "本季最重要的一处纠正，也是最好的教学素材。课程清单标的是 VGG，但打开正文，一级标题是 Going Deeper with Convolutions，主角是 Inception、也就是 GoogLeNet，作者是 Google 的 Szegedy 等人；而且语料库里根本没有 VGG 的一手论文。可是读者在 96 篇 SLAM 论文里见到的是 VGG-16，所以本课的处理范式是：主线讲 GoogLeNet，另开一个前置补丁小节交代 VGG 是谁，VGG 的每一个数字都标转引（来源是 ResNet 篇与 YOLO-SLAM 的 Table 1），绝不写 VGG 论文里说这种句式。含三张自绘图：Inception 模块一层里四条并行路再按通道维拼接、1×1 卷积同时降维又添非线性、以及两条线的对比。并把这次清单失误本身写成教学点——连课程清单都会把两个经典网络记混，读者更不能凭名字猜内容。"
    },
    {
      id: "0188",
      path: "lessons/0188-ResNet-2016-残差连接.html",
      paper: "stage-j",
      title: "ResNet（2016）：残差连接",
      subtitle: "深了反而更差，一条抄近道解决",
      emoji: "🪜",
      duration: "60 分钟",
      tags: ["单篇", "骨干", "残差", "重点"],
      description: "必须讲清一个反直觉现象：退化。56 层的训练误差居然比 20 层更高，而且不是过拟合，是连训练集都学不好。更要紧的是点明 ResNet 的问题不是梯度消失——梯度问题已经被 BatchNorm 与更好的初始化基本解决，剩下的坑是退化。含一条绕过两层的抄近道箭头图：输入 x 一路走两层卷积得到 F(x)，另一路直接把 x 送过去相加，并讲清没得学的时候把权重推成 0 就是恒等映射，这很容易。另含深度退化现象的曲线图（横轴层数、纵轴训练误差）与 bottleneck 的形状账。回链第三季 ORB-SLAM 与第七季的学习式方法：CNN-SLAM 把 ResNet-50 当编码器、TartanVO 用 ResNet-50 当位姿网络。"
    },
    {
      id: "0189",
      path: "lessons/0189-ResNeXt-2017-不加深也能变强.html",
      paper: "stage-j",
      title: "ResNeXt（2017）：不加深也能变强",
      subtitle: "同算力下另开基数这一维",
      emoji: "🧮",
      duration: "55 分钟",
      tags: ["单篇", "骨干", "分组卷积"],
      description: "ResNet 之后，深度与宽度这两维的边际收益都开始递减。ResNeXt 的论点是反命题式的：与其继续加深、加宽，不如把一次大卷积切成 32 份互不通信的小卷积并联，另开一个维度——基数 cardinality。含那张关键对照表：在 2 倍复杂度下，加深只降 0.3、加宽降 0.7，而加基数降 1.3。另含分组卷积与等价形变的示意图，以及本课的形状账。本课还顺手交代了残差连接其实是贯穿整条骨架线的公共零件，不是 ResNet 的专属发明。"
    },
    {
      id: "0190",
      path: "lessons/0190-FCN-2015-全卷积与逐像素.html",
      paper: "stage-j",
      title: "FCN（2015）：全卷积与逐像素预测",
      subtitle: "网络为什么能吐出一张贴满答案的图",
      emoji: "🐱",
      duration: "60 分钟",
      tags: ["单篇", "分割", "全卷积"],
      description: "逐像素预测这个念头的源头。核心是一张并排对照图：左边是全连接层把整张图压成一个向量，右边是输出一张与原图同尺寸、每个像素都标了类别的热力图——要一眼看出为什么全连接层会丢掉空间信息。本课还讲清一个容易被略过的关键：读者真正会用到 FCN 的地方是它的 skip architecture，也就是深层语义与浅层细节的融合，SemanticFusion、CNN-SLAM 与 Mask R-CNN 的掩码分支都建立在这个思想上；只讲全卷积化会让人以为它只是个技巧，而它其实回答了精度从哪来。另含一张全批最重要的区分图：一张图里有两只挨着的猫，用目标检测、语义分割、实例分割三种输出并排对比。回链第八季语义建图块。"
    },
    {
      id: "0191",
      path: "lessons/0191-YOLO-2016-一步到位的检测.html",
      paper: "stage-j",
      title: "YOLO（2016）：一步到位的检测",
      subtitle: "把整条两阶段流水线扔掉",
      emoji: "🚗",
      duration: "60 分钟",
      tags: ["单篇", "检测", "单阶段"],
      description: "把先提一堆候选框、再一个个判整条流水线扔掉，只留一个网络——这一步为什么能快几十倍。含一张不可替换的视觉锚点：真实街景图叠上 7×7 网格，画出几个物体框，标出每个格子只负责中心落在它里面的那个物体，每格输出有没有物体、框的四个数、以及类别。另含两阶段与单阶段的流水线对比图，说明下路正是 Focal Loss 要解决的那个难训问题。速度账是 45 帧每秒、63.4 mAP，Fast YOLO 到 155 帧。本课如实说明原文的损失公式在转换出的 Markdown 里已经严重损坏（求和上下限、指示函数、权重符号都被破坏），所以不直接引用，改用按正文文字重建的版本并显式标注。回链第八季动态块：这是 SLAM 圈出现频率最高的检测零件。"
    },
    {
      id: "0192",
      path: "lessons/0192-MaskRCNN-2017-实例分割与RoIAlign.html",
      paper: "stage-j",
      title: "Mask R-CNN（2017）：实例分割与 RoIAlign",
      subtitle: "SLAM 论文里出现频率最高的零件",
      emoji: "⚙️",
      duration: "70 分钟",
      tags: ["单篇", "分割", "实例分割", "重点"],
      description: "整套课里在 SLAM 论文中出现频率最高的一个零件。含三张自绘图：第一条是它在 Faster R-CNN 旁并联出的第三条掩码分支，并讲清掩码用逐类 sigmoid、分类用 softmax 这个解耦，那 +5.5 AP 就来自这里；第二条是像素格与采样点的错位图，RoIPool 有量化取整、RoIAlign 用双线性插值，在 stride 32 时带来 +7.3 mask AP 与 +10.5 mask AP75，约五成相对提升——这是全季最好的工程细节有多值钱的案例；第三条是掩码分支本身就是一个小 FCN，回链上一课。也如实讲它的摩擦：第八季动态块里 DynaSLAM 用它分割 19 类潜在动态物体并引了 195 毫秒一帧的耗时，MaskFusion 直言它跑不到帧率、边界不精。"
    },
    {
      id: "0193",
      path: "lessons/0193-FocalLoss-2017-九成都是背景.html",
      paper: "stage-j",
      title: "Focal Loss（2017）：九成都是背景怎么办",
      subtitle: "给已答对的样本发折扣券",
      emoji: "⚖️",
      duration: "65 分钟",
      tags: ["单篇", "检测", "损失函数", "重点"],
      description: "全季最抽象的一节：一篇没有图片输出、只有一个公式的论文。含一张不可替换的天平图：一边是成千上万个一眼就对的背景样本，一边是寥寥几个难分的正样本，天平严重倾斜，训练被背景淹没。再把调制因子讲成人话——样本已经分对的时候它接近 0，损失几乎被忽略，只有分错的难样本才保留大权重；原文用的是 γ 等于 2、α 等于 0.25。另含折扣券曲线图（横轴 p_t、纵轴调制因子，画 γ 等于 0、1、2、5 四条），以及一个具体数字：p_t 等于 0.9 的容易样本在 γ 等于 2 时权重被压到原来的百分之一。被引用度据实写——本语料库 255 篇里没有任何一篇 SLAM 论文正面引用 Focal Loss，唯一实质引用来自那篇 2023 年自动驾驶三维检测综述，所以正确的讲法是它是一个被隐式使用的零件。"
    },
    {
      id: "0194",
      path: "lessons/0194-Swin与PVT-2021-把金字塔搬进Transformer.html",
      paper: "stage-j",
      title: "Swin 与 PVT（2021）：把金字塔搬进 Transformer",
      subtitle: "同目标、反路径的一对",
      emoji: "🪟",
      duration: "65 分钟",
      tags: ["单篇", "骨干", "Transformer", "纠正"],
      description: "两篇同年、同目标、反路径的论文，是本研究线里信息密度最高的一对。Swin 把注意力关进窗户再让窗户整体错开半格，靠移位接回跨窗信息，卖的是线性复杂度；PVT 不关窗户，只把 K 与 V 压小，卖的是全局感受野，原文原话是它总能产生全局感受野。含三张自绘图：ViT 的柱状结构与金字塔结构对比、移位窗口的连环画（第一层只连窗内、第二层窗户错位后原来的跨窗邻居同窗了）、以及多头注意力的 K/V 与空间缩减注意力的 K/V 并排。本课必须先说清一处纠正：清单标的是 PVTv2，正文其实是 PVT v1，标题少了结尾的 without Convolutions；PVTv2 不在语料库里，它的线性 SRA、卷积前馈、重叠 patch embedding 一个字都不能出现。并如实写出标题张力：v1 拿免卷积当招牌，续作 v2 又把卷积加回来了，说明免卷积不是终局。回链第七季地点识别块里 VGG-16 加 Swin-B 的混合主干。"
    },
    {
      id: "0195",
      path: "lessons/0195-LiDAR硬件-2017-点云怎么来的.html",
      paper: "stage-j",
      title: "LiDAR 硬件（2017）：点云是怎么从硬件里出来的",
      subtitle: "测距就是测时间",
      emoji: "📡",
      duration: "60 分钟",
      tags: ["单篇", "硬件", "ToF"],
      description: "前八季我们把点云当输入用了几十次，却从没问过它到底是怎么被量出来的。本课只讲两环：第一，测距本质是测时间，距离等于光速乘往返时延除以二——含一张真实旋转式激光雷达向远处砖墙发一束激光的图，标出发射时刻、回波时刻与公式，并用一个数字让人感受到难度：要分辨 1 厘米，需要分辨出 0.067 纳秒。第二，所以每个点自带一个独立的、互不相同的时刻戳。后面关于运动畸变的那一环属于第四季激光主干课，本课只用一小段把它桥接过去，并明确标注这一步原文没讲、是我们的分析。另含三种调制方式并排图（脉冲式靠时延本身、幅度调制连续波靠相位差、频率调制连续波靠拍频）。三处红线如实守住：原篇没有最大测距、扫描频率、功耗的任何数值，没有 MPE 与激光安全等级，也没有运动畸变与时间数字转换器。"
    },
    {
      id: "0196",
      path: "lessons/0196-3D检测综述-2023上-点体素与BEV.html",
      paper: "stage-j",
      title: "三维检测综述（2023）上：点、体素与 BEV",
      subtitle: "点装进什么容器",
      emoji: "📦",
      duration: "60 分钟",
      tags: ["单篇", "综述", "三维检测"],
      description: "上一课拿到点云，这一课回答在一片点里怎么框出那辆车——而答案可以浓缩成一句：先决定这些点被装进什么容器。含一张核心图：同一辆真实汽车的点云，三图并排，左是直接在用点、中是装进体素小方块、右是从天上往下压成 BEV，每图下面标出保留了什么、丢掉了什么。要回链第二季的空间表示与第八季的语义地图，点明这三图与 SLAM 里地图怎么存是同一个问题。另含基于点那条路的代价图：最远点采样一个一个串行地选，原文说它本质是串行算法、无法高度并行，所以推理慢；再画 ball query 的半径旋钮，半径太小信息不够、太大细节被抹平。本课特别强调综述的分类体系是两根轴，常见的点、体素、BEV、多模态融合四条其实跨了两根轴，分类树画歪就是因为没分清这一点。"
    },
    {
      id: "0197",
      path: "lessons/0197-3D检测综述-2023下-多模态与度量.html",
      paper: "stage-j",
      title: "三维检测综述（2023）下：多模态融合与度量",
      subtitle: "融在哪一步·分数由谁定",
      emoji: "🧭",
      duration: "60 分钟",
      tags: ["单篇", "综述", "多模态", "度量"],
      description: "点装进容器只是做了一半，剩下两件事同样要命：相机什么时候插进来，以及这个框到底算不算对由谁说了算。含三张自绘图：第一张是融合位置的时间轴，标出早融合、中间融合、晚融合分别插在流水线的哪一步；第二张是三维框的参数拆解，讲清为什么是七个参数、为什么没有横滚与俯仰，原文写明那个角度是地面上的偏航角；第三张是 AP_3D 与 AP_BEV 的并排对照，同一个框在两个口径下分数不同，因为一个是三维交并比、一个是俯视投影交并比。度量部分要讲清 AP 是在多个交并比阈值下取平均、AP75 比 AP50 更严，并回链第九季 TUM 那课：看到 mAP 先问交并比阈值是多少，正如第九季提醒看到轨迹误差先问真值多准。全程守住一条红线：原综述全文零次出现 SLAM，任何关于它指出与 SLAM 关系的说法都是伪造，只能写成我们的分析。"
    },
    {
      id: "0198",
      path: "lessons/0198-收官-零件地图与名词速查手册.html",
      paper: "stage-j",
      title: "收官：零件地图与名词速查手册",
      subtitle: "两张表·三条判据·一份诚实清单",
      emoji: "🧰",
      duration: "60 分钟",
      tags: ["收官", "基础零件", "总收束"],
      description: "十一篇论文、十三节课，最后收成两张表、三条判据和一份诚实清单。第一张表是名词速查手册，把本季涉及的全部术语逐条列出（backbone、shortcut、cardinality、FCN、RoIAlign、anchor、proposal、mAP、IoU、BEV、voxel、FPS、ToF、AMCW、FMCW 等等），每词给一句大白话加在哪节课讲过，要能当工具用而不是摆设。第二张表是零件地图总表，四个家族乘以解决什么、代表作品、关键数字、在 SLAM 里干什么。再正面回答导览课立的问题：看到一个陌生的词，怎么在三十秒内判断需不需要读懂它，给出可操作的判据，例如它出现在方法主体里，还是只在实验设置、相关工作、或一张表里。最后诚实分区，标出哪些结论有原文证据、哪些只是判断，并重申三处红线——VGG 是转引、PVTv2 不在语料库、硬件篇没有运动畸变与时间数字转换器。"
    },
    {
      id: "0199",
      path: "lessons/0199-导览-第十一季开场-SLAM站在系统的哪一环.html",
      paper: "stage-k",
      title: "导览：第十一季开场——SLAM 站在系统的哪一环",
      subtitle: "先把整台机器拆开看一眼",
      emoji: "🧭",
      duration: "60 分钟",
      tags: ["导览", "应用与相邻领域", "系统链"],
      description: "第十一季开场，也是全项目最后一个横切面的导览。前十季一直在打磨一台机器上的某个零件，这一季终于把整台机器拆开看一眼。核心是一张七环系统链图：传感器、感知前端、SLAM 与里程计、地图、规划、控制、执行器——每一环都画成真实的东西（相机与激光雷达、带色块的感知帧、一条轨迹、点云、绕开障碍的路线、舵量、螺旋桨）。图上再叠三个盖板，标出本季三篇应用论文各覆盖哪几环，一眼就能看出覆盖深度极不均匀：一篇横跨六环、一篇一环都不实现、一篇只管两环的算力部署。导览还兜了链上每个环节「一句话是什么」，并预告两处纠正：一篇被错误当成仿真论文、一篇被清单替它说了它没说过的话。"
    },
    {
      id: "0200",
      path: "lessons/0200-Falcon4-2022-把SLAM塞进一架真飞机.html",
      paper: "stage-k",
      title: "Falcon 4（2022）：把 SLAM 塞进一架真飞机",
      subtitle: "密林树冠下飞完 10 公里",
      emoji: "🚁",
      duration: "70 分钟",
      tags: ["单篇", "系统", "语义SLAM", "重点"],
      description: "全季唯一自洽闭环的整机系统，也是唯一落到执行器的一篇。一架 4.2 公斤四旋翼，在无 GPS 的密林树冠下飞约 10 公里、建出 10897 棵树的语义地图。题眼藏在清单漏掉的副标题里：Under Dense Forest Canopy——为什么树冠下必须用 LiDAR 而不是纯视觉？因为阳光透过树叶的斑驳光斑会破坏视觉的恒亮假设。含三张关键自绘图：双参考系解耦（VIO 局部准但越走越偏、SLAM 长期准但一跳一跳不平滑，只把两者的漂移量注入回路，于是同时保住平滑与无漂）、三层地图的存储量级对照（语义层约 2 MB 对全局粗体素图 1250 MB，约 600 倍）、以及树冠光斑破坏恒亮假设。回链第八季语义建图块、第五季融合、第三季 VIO。"
    },
    {
      id: "0201",
      path: "lessons/0201-运动规划综述-2022-定位之后怎么走.html",
      paper: "stage-k",
      title: "运动规划综述（2022）：定位之后怎么走",
      subtitle: "导航栈的解剖图",
      emoji: "🗺️",
      duration: "60 分钟",
      tags: ["单篇", "综述", "规划"],
      description: "SLAM 把「我在哪」答完了，下一步是「我该往哪走」。这一篇不造任何东西，却把整条导航栈拆开放在桌上。开课先纠正清单两处：它标题结尾是 using machine learning: a survey，所以它是纯综述而不是系统类偏工程；而且它是本批公式最少的一篇，全文只有一个编号式。含系统链上的定位图（它一环都不实现，是元层面的选型指南）、全局规划与局部规划的对照（借用原文的类比：从卧室走到街心公园）、以及代价函数的旋钮图（到达误差、撞上去的惩罚、太慢的惩罚、动作太猛的惩罚各一个旋钮，哪个方案更优取决于你怎么拧）。全季最重要的结论是对第七季的最强告诫：统计 74 篇后发现 43 篇端到端、31 篇非端到端，46 篇做经典问题只有 8 篇有提升，12 篇有安全保证，73 篇仍需人工调参——学习越窄越有效。"
    },
    {
      id: "0202",
      path: "lessons/0202-RecSLAM-2022-算力不够就往上送.html",
      paper: "stage-k",
      title: "RecSLAM（2022）：算力不够就把计算送出去",
      subtitle: "机器人只打包，边缘来算",
      emoji: "🛰️",
      duration: "65 分钟",
      tags: ["单篇", "系统", "边缘计算", "纠正"],
      description: "开课即纠正清单的一处误读：清单说它是「边缘计算加速多机器人仿真」，但真实标题是 Multirobot Simultaneous Localization and Mapping——文件名里的 _Sim 是 Simultaneous 被 66 字符截断剩下的残骸，正好切在第三个字母上，于是 Simultaneous 变成了 Sim，又被顺理成章读成了 simulation。而且它不只是仿真，有真机原型（三台 TurtleBot3 加一个边缘节点，端侧用 Jetson TX2）。含四段时延并排图（打包、传输、计算、融合四段画成一根总条，再把机上算与送到边缘算两种方案并排，让人看出边缘计算不是白赚的，它是用传输换算力，最高降 39.31% 时延）、三级融合链套娃图，以及一张 onboard 与 offboard 的对照图——它和上一课的 Falcon 4 形成正面冲突：算力不够有两条方向相反的解法。要讲清它只覆盖 SLAM 与地图两环的算力部署与融合方式，算法零改动（用 GMapping 当黑盒）。"
    },
    {
      id: "0203",
      path: "lessons/0203-Wildcat-2022-连续时间的激光惯性SLAM.html",
      paper: "stage-k",
      title: "Wildcat（2022）：连续时间的激光惯性 SLAM",
      subtitle: "与第九季闭环的一课",
      emoji: "🐈",
      duration: "75 分钟",
      tags: ["单篇", "SLAM", "连续时间", "重点"],
      description: "全季的闭环课。第九季说过「Wild-Places 的真值由 Wildcat 自举生成，平均误差 0.3 米」——这一课第一次读 Wildcat 本身，并正面做一次跨季对账：Wildcat 正文给的是 QCAT 绝对误差 FlatPack 均值 0.42 米、SpinningPack 均值 0.34 米（Table II，63 个测绘靶标），所以 0.3 米就是 0.34 米的四舍五入；更要紧的是它不是轨迹误差，而是「地图上这 63 个测绘点准不准」——5 公里级室内外场景根本拿不到轨迹真值。做这次对账不是挑第九季的错，而是示范「数字离开口径就会失真」本身。含连续时间与离散时间的对照图（快速旋转的机器人，激光点落在这段时间的不同位置：离散只能归到某一帧、造成运动畸变，连续时间能给任意时刻插值）、B 样条局部支撑图、63 个靶标与误差直线的图。还纠正清单一处：正文里 SE(2) 出现 0 次、也没有稠密重建这个说法，全程用的是 SE(3)，并说清为什么不替论文补话。另有一个打穿误解的反面：它的纯里程计在 MulRan 上比 LIO-SAM 更差，它赢在鲁棒性与全局一致性。"
    },
    {
      id: "0204",
      path: "lessons/0204-撤稿警示-一篇看起来完全正常的论文.html",
      paper: "stage-k",
      title: "撤稿警示：一篇看起来完全正常的论文",
      subtitle: "怎么识别、核查、处置",
      emoji: "🔍",
      duration: "65 分钟",
      tags: ["单篇", "警示", "学术诚信", "重点"],
      description: "这一节不教你怎么读一篇好论文，而是教你在「零件全对、声明全合规、排版都像样」的论文面前怎么保持清醒。写成操作课四段：起——把它用到的第十季零件逐条列出来打勾（RPN、ROI pooling、ResNet50、Faster-RCNN、KITTI、AP 与 IoU、相机激光雷达标定与投影），读者会本能觉得这篇很正规；承——Declarations 四条逐字抄出来，全部合规，「无资金支持」「无相关利益冲突」；转——回到论文内部查自洽性，同一个 mAP 在正文与表格里分别是 97.13% 与 85.67%、摘要同时给 70 毫秒与 15.8 帧每秒而算术不自洽、训练集明细相加是 4500 而正文写 4400、对比表的引用归属与自家参考文献表系统性对不上、式子里旋转矩阵没进目标函数；合——判据换成可核查性（数据代码是否可得、评测协议是否自洽、数字能否复现），并给出发现之后怎么办。全文最要紧的一句：零件都对、声明都合规、格式都像样，仍不保证整篇可信。另讲一件事：它仍有 98 条 Scopus 引用——撤稿不会自动撤回别人已经引用的行为。撤稿理由本课不写（通知是另一份文件）。"
    },
    {
      id: "0205",
      path: "lessons/0205-收官-全季总图与结业.html",
      paper: "stage-k",
      title: "收官：全十一季总图与结业",
      subtitle: "全项目最后一节",
      emoji: "🎓",
      duration: "70 分钟",
      tags: ["收官", "应用与相邻领域", "总收束"],
      description: "全项目最后一节。十一季、十一个阶段、205 节课走到这里。这一节不读新论文，只做三件事：摊开地图、结清账目、说清边界。摊开地图——画出全十一季总图：主干七层（综述与全景、打地基、视觉主干、激光主干、多传感器融合、神经隐式与 3D 高斯、学习式方法）是一条约纵向的技术演进，四个横切面（专题支线、数据集与评测、基础零件、应用与相邻领域）从旁边穿过，是四种不同的切法；并回链第一季的全景纲领，说明从全景出发、最后回到全景。结清账目——给一张十一季速查表（季、阶段、篇数、节数、一句话主题、课号范围），并把「你现在能做什么」写成可检验的能力，同时诚实列出还欠什么：建图质量的评测只被点过名、多机数据集真值难做、有几处「重要性来自行业共识而非本语料库证据」。说清边界——这是 255 篇语料库上的课程，不是 SLAM 全集；库里没有的东西要如实说不在库内。"
    },
    {
      id: "QA0002",
